...
use App\Services\Business\Reports\ItemsSoldReportService;
use App\Exports\ItemSoldReportExport;
use Maatwebsite\Excel\Facades\Excel;
use Carbon\Carbon;

class ItemsSoldReport extends Component
{
    use WithPagination;
     ...
     */
    public function downloadItemsSoldReport()
    {
        $export = app(ItemSoldReportExport::class, [
            'filters' => [
                'search' => $this->search,
        ...
        $filename = 'items_sold_report_' . now()->format('Y_m_d') . '.xlsx';
        return Excel::download($export, $filename);
    }

    public function render()
    {
        /** @var ItemsSoldReportService $service */
        $service = app(ItemsSoldReportService::class);
        $records = $service->paginateItems(
        ...
        return view('livewire.business.reports.items-sold-report', [
            'records' => $records,
        ]);
    }
