...
const validation = {
    ...
    notes_max: 'Character limit exceeded.',
    ...
    inactive_confirm: 'Are you sure you want to mark this maintenance order as inactive?',
...
window.Alpine?.data('maintenanceOrderForm', () => ({
    // Form state
    ...
    initializeField(fieldName) {
        ...
        return oldValue ?? workOrderValue ?? '';
    },
    ...
    // Trim form values
    trimFormValues() {
        // Update the form input values with trimmed versions
        if (this.jobName) {
            this.jobName = this.jobName.trim();
            const nameInput = this.$refs.form?.querySelector('[name="name"]');
            if (nameInput) nameInput.value = this.jobName;
        }

        if (this.jobDescription) {
            this.jobDescription = this.jobDescription.trim();
            const descInput = this.$refs.form?.querySelector('[name="description"]');
            if (descInput) descInput.value = this.jobDescription;
        }

        if (this.additionalTask) {
            this.additionalTask = this.additionalTask.trim();
            const taskInput = this.$refs.form?.querySelector('[name="additional_task"]');
            if (taskInput) taskInput.value = this.additionalTask;
        }
    },
    ...
    submitWithAction(action) {
        try {
            ...
            // Check if this is an update operation (existing maintenance order)
    ...
    validateChecklistItemsPresent() {
        // For maintenance, checklist items are sent as checklist_items[*][item_text]
        const anyItem = document.querySelector('input[name^="checklist_items["][name$="[item_text]"]');
        if (!anyItem) {
    ...
    validateJobDetails() {
        const trimmedName = this.jobName ? this.jobName.trim() : '';
        const trimmedDescription = this.jobDescription ? this.jobDescription.trim() : '';
        ...
        if (trimmedDescription && trimmedDescription.length > VALIDATION_RULES.DESCRIPTION_MAX_LENGTH) {
    ...
    validateOptionalFields() {
        const trimmedAdditionalTask = this.additionalTask ? this.additionalTask.trim() : '';
        
        if (trimmedAdditionalTask && trimmedAdditionalTask.length > VALIDATION_RULES.ADDITIONAL_TASK_MAX_LENGTH) {
    ...
    validateRepeatAfter() {
        ...
        const maxValue = maxValues[this.frequency];
        if (maxValue && this.repeatAfter > maxValue) {
            this.errors.repeat_after = `${this.frequency.charAt(0).toUpperCase() + this.frequency.slice(1)} frequency cannot exceed ${maxValue} ${this.frequency === 'weekly' ? 'weeks' : 'months'}.`;
            return false;
        }
    ...
    validateRecurringFields() {
        ...
        const validations = [
            ...
            this.validateRepeatAfter,
            this.validateSelectedDays,
            this.validateMonthlyDay
        ];
...
window.Alpine?.data('photoUpload', () => ({
    ...
    init() {
        ...
        if (window.oldWorkOrder?.photo) {
            ...
            this.existingPhotoThumbUrl = window.oldWorkOrder.photo_thumb_url ?? window.oldWorkOrder.photo_url;
