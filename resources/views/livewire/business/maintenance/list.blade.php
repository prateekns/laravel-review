<div class="container-fluid mx-auto" x-data="{ search: '' }">
    ...
    <div class="flex justify-between items-center flex-row max-[1200px]:flex-col add-sub-mobile-wrapper">
        <div class="gap-[16px] flex flex-col">
            <h1 class="main-heading">{{ __('business.maintenance.list_title') }}</h1>
            <p class="sub-heading">{{ __('business.maintenance.list_description') }}</p>
        </div>
        @if (!$maintenanceOrders->isEmpty())
            <div class="flex gap-[20px] max-[600px]:flex-col max-[600px]:w-full">
                <x-form.link link="{{ route('business.work-orders.maintenance.create') }}"
                    class="btn-box btn max-[600px]:w-full">
                    {{ __('business.maintenance.create_button') }}
                </x-form.link>
            </div>
        @endif
    </div>
    ...
    @if ($maintenanceOrders->isEmpty() && !$search)
        <x-empty-state :title="__('business.maintenance.empty_state.title')" :description="__('business.maintenance.empty_state.description')" :buttonText="__('business.maintenance.create_button')" :buttonLink="route('business.work-orders.maintenance.create')"
            ...
            <div class="white-box">
                <div class="top-box">
                    <div class="sm:flex-auto">
                        <h2 class="white-box-heading">{{ __('business.maintenance.existing') }}</h2>
                    </div>
                    ...
                    <div class="grid grid-cols-1 relative">
                        <input type="text" wire:model.live="search"
                            ...
                            aria-label="{{ __('business.work_orders.search') }}">
                        ...
                        @if ($search)
                            <button type="button" wire:click="$set('search', '')"
                                ...
                                aria-label="{{ __('business.maintenance.clear_search') }}">
                ...
                <div class="table-box">
                    <table class="min-w-full divide-y divide-gray-300" aria-describedby="Maintenance Order List">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">{{ __('business.maintenance.table.frequency') }}</th>
                        ...
                        <tbody>
                            @forelse($maintenanceOrders as $order)
                                <tr>
                                    <td data-label="Work Order ID">{{ $order->id }}</td>
                                    <td data-label="Customer Name">{{ !empty($order->customer->full_name) ? $order->customer->full_name : '-' }}</td>
                                    <td data-label="Commercial Name">{{ $order->customer->commercial_pool_details ?? '-' }}</td>
                                    <td data-label="Name"
                                        class="text-wrap truncate overflow-ellipsis max-w-[200px] max-[640px]:max-w-[100%]"
                                        title="{{ $order->name }}">
                                        <a href="{{ route('business.work-orders.maintenance.show', $order) }}"
                                            title="{{ __('business.maintenance.view') }}">
                                            {{ $order->name }}
                                        </a>
                                    </td>
                                    <td data-label="Frequency">
                                        {{ $order->is_recurring ? __('business.maintenance.frequency.' . $order->frequency) : __('business.maintenance.non_recurring') }}
                                    </td>
                                    <td data-label="Next Occurrence">
                                        {{ $order->next_occurrence ?? 'N/A' }}
                                    </td>
                                    ...
                                    <td class="table-actions">
                                        ...
                                        @if ($order->canBeEdited())
                                            <a href="{{ route('business.work-orders.maintenance.edit', $order) }}"
                                                class="text-[#0d44ea]"
                                                title="{{ __('business.maintenance.edit') }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="py-4 text-center text-gray-500">
                                        @if ($search)
                                            <span
                                                class="w-full inline-block text-center">{{ __('business.maintenance.no_records') }}</span>
                                        @else
                                            <span
                                                class="w-full inline-block text-center">{{ __('business.maintenance.empty_state.description') }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
                    ...
                    @if ($maintenanceOrders->total() > 10)
                        <div class="pagination-box">
                            <x-pagination :paginator="$maintenanceOrders" />
                        </div>
                    @endif
                </div>
            </div>
