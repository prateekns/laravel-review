...
<div class="bg-[#FFFAE8] border border-[#DFB400] text-[14px] font-[500]  text-[#212529] rounded-lg p-[24px] w-full mt-[28px]" id="plan-info-banner">
    <div class="flex items-start flex-wrap gap-x-6 gap-y-2 flex-col">
        ...
        <div class="flex items-center gap-3">
            ...
            <span>
                Your Current Billing Supports {{ $user->business->num_admin }} admins, {{ $user->business->num_technician }} technician
            </span>
        </div>
        ...
        <div class="flex-shrink-0">
            @if($subscription->ends_at)
                ...
                <span class="text-[#B32318]">{{ App\Helpers\Helper::getFormattedDate($subscription->ends_at) }}</span>
            @else
            <span>
                {{ __('Next payment:') }}
                <span class="text-[#B32318]">
                    {{ $amountDue }} on {{ $nextPaymentDate }}
                </span>
            </span>
            @endif
        </div>
