...
@section('content')
    <div class="container-fluid mx-auto">
        ...
        <x-page-heading title="{{ __('business.work_orders.view_details') }}"
            ...
            @if ($workOrder->technician)
                <div class="flex gap-2 flex-col text-left items-start max-[767px]:w-full max-[767px]:mt-[12px]">
                    ...
                    <span class="text-[16px]">{{ $workOrder->current_technician->full_name }}</span>
                </div>
            ...
            @else
                <div class="flex items-center gap-2 max-[767px]:w-full max-[767px]:justify-start max-[767px]:mt-[16px]">
                    <x-form.link link="{{ route('business.scheduler.assign', ['workOrder' => $workOrder->id]) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-1">
                            ...
                            <div>
                                ...
                                <p class="mt-[4px] text-[16px] font-[400] text-[#000000]" id="customer-name">
                                    {{ $workOrder->isCompleted() ? $completedJobCustomer->name ?? '-' : $workOrder->customer->customer_name ?? '-' }}
                                </p>
                            </div>
                            ...
                            <div class="col-span-2">
                                @if ($workOrder->isCompleted())
                                    @include(
                                        ...
                                        [
                                            'completedJobCustomer' => optional($workOrder->completed_job_customer),
                                            'hasNoPoolDetails' => $workOrder->has_no_pool_details,
                                        ]
                                    )
                                @else
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-[24px] ">
                            ...
                            <div class="task-details-box relative">
                                ...
                                <div class="mt-0">
                                    ...
                                    <p class="mt-[4px] text-[16px] font-[400] text-[#000000]">{{ $workOrder->name }}</p>
                                </div>

                                @if ($workOrder->additional_task)
                                    <div class="mt-[24px]">
                                        <x-form.label
                                            class="!text-[14px] !font-[400] !text-[#000000]">{{ __('business.work_orders.additional_task') }}</x-form.label>
                                        <p class="mt-[4px] text-[16px] font-[400] text-[#000000]">
                                            {{ $workOrder->additional_task }}</p>
                                    </div>
                                @endif
                                ...
                                <div class="mt-[24px]">
                                    ...
                                    <p class="mt-[4px] text-[16px] font-[400] text-[#000000]">
                                        {{ $workOrder->preferred_start_date_formatted ? Carbon\Carbon::parse($workOrder->preferred_start_date_formatted)->format('M d, Y') : '-' }}
                                    </p>
                                </div>
                                ...
                                <div class="mt-[24px]">
                                    ...
                                    <p class="mt-[4px] text-[16px] font-[400] text-[#000000]">
                                        {{ $workOrder->preferred_start_time_formatted ?: '-' }}
                                    </p>
                                </div>
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-center gap-3">
                                                <div class="w-12 h-12 rounded-lg overflow-hidden">
                                                    <img src="{{ $workOrder->photo_thumb_url }}" alt="Uploaded by technician after workorder completion"
                                                        class="w-full h-full object-cover cursor-pointer" x-data
                                                        @click="$dispatch('open-modal', {
                                                        id: 'image-preview-modal',
                                                        url: '{{ $workOrder->photo_url }}'
                                                    })">
                                                </div>
                                                <div>
                                                    <p class="text-[#0B0B0B] font-[600] text-[12px]">
                                                        {{ basename($workOrder->photo) }}</p>
                            ...
                            <div class="task-details-box relative">
                                <div class="bg-white rounded-lg">
                                    ...
                                    @if ($workOrder->checklist && count($workOrder->checklist) > 0)
                            ...
                            <div
                                ...
                                <div class="frequency-box">
                                    ...
                                    <p
                                        ...
                                        {{ $workOrder->frequency_label }}
                                        {{ $workOrder->repeat_after_label ? '(' . $workOrder->repeat_after_label . ')' : '' }}
                                    </p>
                                </div>
                                ...
                                <div class="frequency-box">
                                    ...
                                    <p
                                        class="mt-[12px] text-[16px] max-[767px]:text-[14px] font-[500] text-[#1C1D1D] text-center">
                                        @if ($workOrder->frequency === 'daily')
                                            {{ __('business.maintenance.all_days') }}
                                        @elseif($workOrder->frequency === 'monthly' && $workOrder->monthly_day_type_label && $workOrder->monthly_day_of_week_label)
                                            {{ $workOrder->monthly_day_type_label }}
                                            {{ $workOrder->monthly_day_of_week_label }}
                                        @elseif($workOrder->selected_days_label)
                                ...
                                <div class="frequency-box">
                                    ...
                                    <p
                                        ...
                                        {{ $workOrder->end_date ? Carbon\Carbon::parse($workOrder->end_date_formatted)->format('M d, Y') : '-' }}
                                    </p>
                                </div>
                            </div>
                        </div>
        ...
        <div class="white-box shadow-sm py-[24px] px-[20px] rounded-[16px] mb-4">
            <div x-data="{ open: true }">
                ...
                <div x-show="open" x-transition class="accordian-contant w-full !mt-[30px] !p-[0]">
                    <div class="px-0 pb-6">
                        <!-- Items Sold -->
                        @if ($workOrder->itemsSold && count($workOrder->itemsSold) > 0)
                            <div class="mb-0">
                                <x-form.label class="">{{ __('business.items_sold.title') }}</x-form.label>
                                <div class="table-box !px-0 mt-0">
                                    <table class="min-w-full divide-y divide-gray-300" aria-describedby="Items sold">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach ($workOrder->itemsSold as $item)
                                                <tr>
                        ...
                        @if ($workOrder->extra_work_done)
                            <div class="px-[0px] py-[8px] pt-0">
                                ...
                                <div class="mt-[4px] text-[12px] font-[400] text-[#000000] px-[12px]">
                                    {!! nl2br(e($workOrder->extra_work_done)) !!}</div>
                            </div>
                        @endif

                        @if (!$workOrder->itemsSold || (count($workOrder->itemsSold) === 0 && !$workOrder->extra_work_done))
        ...
        <div class="px-0 pb-6">
            ...
            <p class="text-[14px] font-[500] text-[#1C1D1D]">{{ $workOrder->communication_notes ?: '-' }}</p>
