    ...
    <div class="container-fluid mx-auto">
        ...
        <x-page-heading title="{{ __('business.maintenance.view_details') }}"
            ...
            @if ($maintenance->technician)
                <div class="flex gap-2 flex-col text-left items-start max-[767px]:w-full max-[767px]:mt-[12px]">
                    ...
                    <span class="text-[16px]">{{ $maintenance->current_technician->full_name }}</span>
                </div>
            @elseif (!$maintenance->is_active)
                <div class="inline-flex absolute top-[28px] right-[36px] min-w-[71px]">
                    <span
                        class="customer-status-badge w-full px-[7px] py-[2px] rounded-[4px] text-[12px] font-[600] text-center bg-[#E43232] text-white">
            ...
            @else
                <div class="flex items-center gap-2 max-[767px]:w-full max-[767px]:justify-start max-[767px]:mt-[16px]">
                    <x-form.link link="{{ route('business.scheduler.assign', ['workOrder' => $maintenance->id]) }}"
                        class="btn-box btn max-[767px]:w-full whitespace-nowrap" id="assign-technician-btn">
                        {{ __('business.work_orders.assign_technician') }}
                    </x-form.link>
                </div>
            @endif
        </x-page-heading>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-0">
                            ...
                            <div>
                                ...
                                <p class="mt-[4px] text-[16px] font-[400] text-[#000000]" id="customer-name">
                                    {{ $maintenance->isCompleted() ? $completedJobCustomer->name ?? '-' : $maintenance->customer->customer_name ?? '-' }}
                                </p>
                            </div>
                            ...
                            <div class="col-span-2">
                                @if ($maintenance->isCompleted())
                                    @include(
                                        ...
                                        [
                                            'completedJobCustomer' => optional(
                                                $maintenance->completed_job_customer),
                                            'hasNoPoolDetails' => $maintenance->has_no_pool_details,
                                        ]
                                    )
                                @else
                    ...
                    <div class="px-0 pb-6 max-[767px]:px-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-[24px]">
                            ...
                            <div class="task-details-box relative">
                                ...
                                @if ($maintenance->additional_task)
                                    <div class="mt-[24px]">
                                        <x-form.label
                                ...
                                <div class="mt-[24px]">
                                    ...
                                    <p class="mt-[4px] text-[16px] font-[400] text-[#000000]" id="preferred-start-date">
                                        {{ $maintenance->preferred_start_date_formatted ? Carbon\Carbon::parse($maintenance->preferred_start_date_formatted)->format('M d, Y') : '-' }}
                                    </p>
                                </div>
                                ...
                                <div class="mt-[24px]">
                                    ...
                                    <p class="mt-[4px] text-[16px] font-[400] text-[#000000]" id="preferred-start-time">
                                        {{ $maintenance->preferred_start_time_formatted ?: '-' }}
                                    </p>
                                </div>
                                        ...
                                        <div
                                            ...
                                            <div class="flex items-center gap-3 photo-preview">
                                                <div class="w-12 h-12 rounded-lg overflow-hidden">
                                                    <img src="{{ $maintenance->photo_thumb_url }}" id="uploaded-photos"
                                                        alt="Maintenance Order"
                                                        class="w-full h-full object-cover cursor-pointer" x-data
                                                        @click="$dispatch('open-modal', {
                                                        id: 'image-preview-modal',
                                                        url: '{{ $maintenance->photo_url }}'
                                                    })">
                                                </div>
                                                <div>
                                                    <p class="text-[#0B0B0B] font-[600] text-[12px] filename">
                                                        {{ basename($maintenance->photo) }}</p>
                            ...
                            <div
                                ...
                                <div class="frequency-box">
                                    ...
                                    <p
                                        ...
                                        {{ $maintenance->frequency_label }}
                                        {{ $maintenance->repeat_after_label ? '(' . $maintenance->repeat_after_label . ')' : '' }}
                                    </p>
                                </div>
                                    ...
                                    <p
                                        ...
                                        @elseif(
                                            $maintenance->frequency === 'monthly' &&
                                                $maintenance->monthly_day_type_label &&
                                                $maintenance->monthly_day_of_week_label)
                                            {{ $maintenance->monthly_day_type_label }}
                                            {{ $maintenance->monthly_day_of_week_label }}
                                        @elseif($maintenance->selected_days_label)
                                            {{ $maintenance->selected_days_label }}
                                        @else
                                            -
                                        @endif
                                    </p>
                                ...
                                <div class="frequency-box">
                                    ...
                                    <p
                                        ...
                                        {{ $maintenance->end_date ? Carbon\Carbon::parse($maintenance->end_date_formatted)->format('M d, Y') : '-' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            @if ($regularChemicalLogs->count() > 0)
                                ...
                                <div class="mb-0">
                                    ...
                                    <div class="table-box !px-0 !pb-0 !mt-[16px] !pt-0">
                                        <table class="min-w-full divide-y divide-gray-300" aria-describedby="Maintenance Order">
                                            <thead>
                                                <tr>
                                                    <th scope="col">
                                                        {{ __('business.work_orders.chemical_name_value') }}</th>
                                                    ...
                                                    <th scope="col">{{ __('business.work_orders.chemical_added') }}
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody>
                            ...
                            @if ($additionalItemLogs->count() > 0)
                                <div class="mt-6">
                                    ...
                                    <div class="table-box !px-0 !pb-0 !pt-0 !mt-[16px]">
                                        <table class="min-w-full divide-y divide-gray-300" aria-describedby="Maintenance Order Items Sold">
                                            <thead>
                                                <tr>
        ...
        <div class="white-box shadow-sm py-[24px] px-[20px] rounded-[16px] mb-4">
            <div x-data="{ open: false }">
                ...
                <div x-show="open" x-transition class="accordian-contant w-full !mt-[30px] !p-[0]">
                    <div class="px-0 pb-0">
                        @if (($maintenance->itemsSold && $maintenance->itemsSold->count() > 0) || $maintenance->extra_work_done)
                            <!-- Items Sold Table -->
                            @if ($maintenance->itemsSold && $maintenance->itemsSold->count() > 0)
                                <div class="mb-0" id="items-sold-table">
                                    ...
                                    <div class="table-box !px-0 !mt-16px !pt-0">
                                        <table class="min-w-full divide-y divide-gray-300" aria-describedby="Maintenance Sold Items">
                                            <thead>
                                                <tr>
                                                    <th scope="col">{{ __('business.work_orders.item_name') }}</th>
                                            ...
                                            <tbody>
                                                @foreach ($maintenance->itemsSold as $item)
                                                    <tr>
                            ...
                            <!-- Additional Work -->
                            @if ($maintenance->extra_work_done)
                                <div class="px-[0px] py-[8px] pt-0 !pb-0 max-[767px]:!mt-[16px]" id="extra-work-section">
                                    ...
                                    <div class="mt-[4px] text-[12px] font-[400] text-[#000000] px-[12px]"
                                        id="extra-work-done">{!! nl2br(e($maintenance->extra_work_done)) !!}</div>
                                </div>
                            @endif
                        @else
        ...
        <div class="px-0 pb-6" id="communication-notes">
            ...
            <p class="text-[14px] font-[500] text-[#1C1D1D]">{{ $maintenance->communication_notes ?: '-' }}</p>
            @include('business.work-orders.parts.customer-attachments', [
